...
<div class="windSend"></div>
	<div class="formSEnd">
		<form action="alfavit.php" method="POST">
...
<div class="elementy-saita" id="content">
 ...
 <div class="box-contener" id="main" >
  <div class="img_prod">
       <img src="image/Макароны/Алфавит.jpg" >
       </div>
     <div class="H2 nameProd">Алфавит</div>
		<div >Описание</div>
		<p id="p">      Макароны Алфавит изготовлены из отборной пшеницы твердых сортов, эксклюзивных натуральных добавок и фильтрованной воды.<br> 

       Созданные специально для детей, макаронные изделия сохраняют свой привлекательный внешний вид и после варки. Малыши учатся правильно жевать и получают от этого процесса пользу и удовольствие.<br> 

Особенности:<br>
* созданные специально для детей, макаронные изделия сохраняют свой привлекательный внешний вид и после варки<br>
* форма макарон в виде букв русского алфавита и три разных цвета обязательно понравятся детям и улучшат аппетит<br>
* малыши учатся правильно жевать и получают от этого процесса пользу и удовольствие<br>
* оригинальная рецептура изделий из специально подобранных натуральных ингредиентов</p>

	<table>
	<tr>
		<td id="strong">Цена</td>
		<td class="priceCol", id="strong"> 120р </td>
	</tr>	
</table>
...
if (isset($_POST["nameProd"])) {
    ...
    $sql = mysql_query("INSERT INTO `mak_order` (`CustName`,`Phone`,`Address`,`NameProd`,`Quantity`,`SumOrder`) VALUES ('".$_POST['name']."','".$_POST['phone']."','".$_POST['address']."','".$_POST['nameProd']."','".$_POST['count']."','".$_POST['lastPrice']."')");
